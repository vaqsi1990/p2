...
router.post('/', upload.single('image'), handleMulterError, async (req, res) => {
    try {
        ...
        const result = await pool.query(
            'INSERT INTO images (name, description, price, image_url) VALUES ($1, $2, $3, $4) RETURNING *',
            [name, description || null, price ? parseFloat(price) : null, imageUrl]
        );
    ...
    try {
        ...
        const result = await pool.query(
            ...
            [
                ...
                price !== undefined ? (price ? parseFloat(price) : null) : existingImage.price,
                imageUrl,
                req.params.id
            ]
        );
